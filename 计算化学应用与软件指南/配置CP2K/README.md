...
***注：如果编译中途报错，并且从后往前找error的时候看到“找不到-lz”的报错提示，则运行sudo apt install zlib1g命令装上zlib库，再重新运行上面的make那行命令即可。若看到“找不到-lsz”，则先运行sudo apt install libsz2装上libsz2库，然后运行sudo ln -s /usr/lib/x86_64-linux-gnu/libsz.so.2 /usr/lib/x86_64-linux-gnu/libsz.so创建其可被ld程序检测到的符号链接,最后重新运行上面的make。***
...
popt：支持MPI并行，这使其不仅可以单机多核并行，还可以实现跨节点并行；但不支持OpenMP并行，因而每个MPI进程独占一份内存。一般运行计算任务时建议默认使用该版本。
